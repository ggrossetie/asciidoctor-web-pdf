...
const asciimath = require('./asciimath.js')
...
class DocumentPDFConverter {
  ...
  constructor () {
    ...
    this.stemContent = require('./katex.js')
...
${this.repeatTableElementsContent}
  ...
  convert_inline_quoted (node) {
    let equation = node.getText()
    let type = node.getType()
    if (type === 'asciimath') {
      // KaTeX only supports LaTeX, convert asciimath to latexmath
      equation = asciimath.toLaTeX(equation)
      type = 'latexmath'
    }
    const [open, close, tag] = QuoteTags[type] || ['', '']
    const idAttribute = node.getId() ? ` id="${node.getId()}"` : ''
    const classAttr = (node.getRoles() && node.getRoles().length > 0) ? ` class="${node.getRoles().join(' ')}"` : ''
    if (idAttribute !== '' || classAttr !== '') {
      if (tag) {
        return `${open.slice(0, -1)}${idAttribute}${classAttr}>${equation}${close}`
      }
      return `<span${idAttribute}${classAttr}>${open}${equation}${close}</span>`
    }
    return `${open}${equation}${close}`
  }
...
${node.getContent()}
  ...
  convert_stem(node) {
    ...
    const open = '\\['
    const close = '\\]'
    let equation = node.getContent() || ''
    if (node.style === 'asciimath') {
      // KaTeX only supports LaTeX, convert asciimath to latexmath
      equation = asciimath.toLaTeX(equation)
      if (equation.includes('\n')) {
        const br = `\n<br/>`
        equation = equation.replace(StemBreakRx, (value) => {
          const count = (value.match(/\\n/g)).length - 1;
          `%(${close}${br.repeat(count)}\n${open})`
        })
      }
    ...
    return `<div${idAttribute} class="stemblock${node.getRole() ? ` ${node.getRole()}` : ''}">
...
module.exports.templates = {
  ...
  outline: (node, opts) => instance.convert_outline(node, opts),
  inline_quoted: node => instance.convert_inline_quoted(node),
  ...
  stem: node => instance.convert_stem(node)
}
