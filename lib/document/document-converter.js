...
class DocumentPDFConverter {
  ...
  constructor() {
    ...
    this.repeatTableElementsContent = fs.readFileSync(
      ospath.join(__dirname, 'repeating-table-elements.js'),
      'utf8',
    )
    this.pagedContent = fs.readFileSync(
      require.resolve('@ggrossetie/pagedjs/dist/paged.polyfill.js'),
      'utf8',
    )
    this.pagedRendering = fs.readFileSync(
      `${__dirname}/paged-rendering.js`,
      'utf8',
    )
    const stylesDirectoryPath = ospath.resolve(`${__dirname}/../../css`)
    this.asciidoctorStyleContent = fs.readFileSync(
      `${stylesDirectoryPath}/asciidoctor.css`,
      'utf8',
    )
    this.documentStyleContent = fs.readFileSync(
      `${stylesDirectoryPath}/document.css`,
      'utf8',
    )
    this.titleDocumentStyleContent = fs.readFileSync(
      `${stylesDirectoryPath}/features/title-document-numbering.css`,
      'utf8',
    )
    this.titlePageStyleContent = fs.readFileSync(
      `${stylesDirectoryPath}/features/title-page.css`,
      'utf8',
    )
    this.bookStyleContent = fs.readFileSync(
      `${stylesDirectoryPath}/features/book.css`,
      'utf8',
    )
  }
...
${this.docinfoContent(node, 'footer', '-pdf.html')}
  ...
  convert_outline(node, opts = {}) {
    ...
    const sectnumlevels =
      opts.sectnumlevels ||
      parseInt(node.getDocument().getAttributes().sectnumlevels) ||
      3
    const toclevels =
      opts.toclevels ||
      parseInt(node.getDocument().getAttributes().toclevels) ||
      2
