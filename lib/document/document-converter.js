...
class DocumentPDFConverter {
  ...
  async convert (node, transform, opts) {
    ...
    if (convertFunction) {
      return await convertFunction.call(this, node)
    }
...
${titleElement}${content}
  ...
  async convert_inline_image (node) {
    if (node.getType() === 'icon' && this.isSvgIconEnabled(node)) {
      ...
      return await this.baseConverter.convert(node, 'inline_image')
    }
  }
  ...
  async convert_colist (node) {
    ...
    if (node.getDocument().hasAttribute('icons') || node.getDocument().isAttribute('icontype', 'svg')) {
      ...
      for (const item of node.getItems()) {
        ...
        const itemContent = item.blocks.length > 0 ? `\n ${await item.getContent()}` : ''
        result.push(`<tr>
          <td>${numLabel}</td>
...
${content}
  ...
  async convert_inline_footnote (node) {
    if (node.getDocument().isAttribute('nofootnotes')) {
      return await this.baseConverter.convert(node, 'inline_footnote')
    }
...
${faDom.css()}
  ...
  syntaxHighlighterHead (node, syntaxHighlighter, attrs) {
    let header = ''
    if (syntaxHighlighter !== null && syntaxHighlighter.hasDocinfo('head')) {
      header = syntaxHighlighter.docinfo('head', node, attrs)
    }
    if (syntaxHighlighter !== null && syntaxHighlighter.hasDocinfo('footer')) {
