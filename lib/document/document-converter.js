...
<p>${item.getText()}</p>${item['$blocks?']() ? `\n ${item.getContent()}` : ''}`)
  ...
  convert_preamble (node, opts = {}) {
    ...
    // this is a workaround for a bug in paged.js to allow the preamble margin content
    // to be removed by a named page. A filler div is required so that css selection
    // works on the preamble div
    // See https://gitlab.pagedmedia.org/tools/pagedjs/issues/311
    let preambleHead
    if (doc.getDoctype() === 'book') {
      preambleHead = '<div id="preamble-book-filler"></div><div id="preamble">'
    } else {
      preambleHead = '<div id="preamble">'
    }
