...
class DocumentPDFConverter {
  ...
  constructor () {
    ...
    this.repeatTableHeadersContent = fs.readFileSync(`${__dirname}/repeating-table-headers.js`, 'utf8')
    this.pagedContent = fs.readFileSync(require.resolve('pagedjs/dist/paged.polyfill.js'), 'utf8')
    ...
    this.titlePageStyleContent = fs.readFileSync(`${stylesDirectoryPath}/features/title-page-numbering.css`, 'utf8')
...
${this.pagedContent}
${this.pagedRendering}
${this.repeatTableHeadersContent}
</script>
</body>
</html>`
  }
...
${this.baseConverter.$convert_outline(doc)}
</div>`
    } else {
      toc = ''
    }
...
${faDom.css()}
  ...
  outline (node) {
    if (this.baseConverter) {
      return `<div style="display: none;">${this.baseConverter.$convert_outline(node)}</div>`
    }
    return ''
  }
...
${this.baseConverter.$convert_outline(node)}
</div>`
      }
    }
    return ''
...
module.exports.templates = {
  document: node => instance.convert_document(node),
