...
class Browser {
  ...
  async _launch (preview) {
    // if creating binaries with pkg, we use bundled chromium instances stored in
    // the `chromium` dir instead of chromium installed as part of the puppeteer package
    const chromiumExecutablePath = (isPkg
      ? puppeteer.executablePath().replace(
          /^.*?\/node_modules\/puppeteer\/\.local-chromium/,
          ospath.join(ospath.dirname(process.execPath), 'chromium')
        )
