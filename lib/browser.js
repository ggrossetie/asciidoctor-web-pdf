...
const getChromiumRelativePath = (revision) => {
  // Returns the platform-specific path to the Chromium binary relative to the
  // "chromium/" download directory, matching puppeteer v15's directory layout.
  const { platform, arch } = process
  if (platform === 'linux') {
    return ospath.join(`linux-${revision}`, 'chrome-linux', 'chrome')
  }
  if (platform === 'darwin') {
    if (arch === 'arm64') {
      return ospath.join(
  ...
  // win32
  return ospath.join(`win64-${revision}`, 'chrome-win', 'chrome.exe')
}
...
const getPuppeteerExecutablePath = () => {
  ...
  if (isSea) {
    ...
    // __dirname is baked to the build machine path. Compute the path directly instead.
    const revision = puppeteer.default._preferredRevision
    return ospath.join(
      ospath.dirname(process.execPath),
      'chromium',
      getChromiumRelativePath(revision),
    )
  }
  if (isPkg) {
    ...
    const partialPath = parts.splice(
      parts.indexOf('.local-chromium') + 1,
      parts.length,
    )
    return ospath.join(
      ospath.dirname(process.execPath),
