...
export async function convert(inputFile, options, timings, watch, preview) {
  ...
  try {
    const viewerUrl = `file://${viewerIndexPath}#src=file://${tempFile}&bookMode=false&renderAllPages=true&spread=false`
    const page = await browserInstance.goto(viewerUrl, preview)
    ...
    if (!preview) {
      ...
      pdfDoc = await addOutline(pdfDoc, doc, `file://${tempFile}`)
