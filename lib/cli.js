...
export class PdfOptions extends Options {
  constructor() {
    ...
    this.addOption('preprocess-scripts', {
      type: 'boolean',
      describe:
        'load the HTML in a browser, wait for scripts to complete (networkidle0), then pass the enriched HTML to Vivliostyle',
    })
...
CLI version ${getAsciidoctorJsVersion()}`
  ...
  async convertFiles(files, options, values) {
    ...
    const config = {
      ...
      preview,
      verbose,
      preserveHtml: values['preserve-html'],
      preprocessScripts:
        values['preprocess-scripts-wait-for'] ?? values['preprocess-scripts'],
    }
    await this._convertAll(files, options, config)
  ...
  async _convertFromStdin(options) {
    ...
    await convertPdf({ path: fictiveInputFile, contents: data }, options, {
  ...
  async _convertAll(files, options, config) {
    for (const file of files) {
      if (config.verbose) console.log(`converting file ${file}`)
      await convertPdf({ path: file }, options, config)
    }
  }
}
