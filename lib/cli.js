...
const DOT_RELATIVE_RX = new RegExp(`^\\.{1,2}[/${path.sep.replace('/', '').replace('\\', '\\\\')}]`)
...
function convertOptions (argv) {
  ...
  if (templateRequireLib) {
    converter.registerTemplateConverter(requireLibrary(templateRequireLib))
  }
  if (verbose) {
...
async function convertFiles (files, argv, options, verbose) {
  for (let file of files) {
    ...
    await converter.convert(file, options, argv['timings'])
  }
}
...
async function run (argv) {
  ...
  if (version || (verbose && files && files.length === 0)) {
    console.log(`Asciidoctor ${converter.getCoreVersion()} [http://asciidoctor.org]`)
